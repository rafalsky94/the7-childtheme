<?php
	...
	if($show_success_notice) :
		...
		<div class="action-tab success">
			<h3><?php printf( 'All Recommended action has been successfully completed.', 'uncode-lite'); ?></h3>
			<a class="button button-primary" href="<?php echo esc_url(admin_url('customize.php')); ?>"><?php echo esc_html__('Customize Theme', 'uncode-lite'); ?></a>
		</div>
		<?php
	else :
		...
		foreach($req_plugins as $plugin) :
			if($plugin['bundled'] == false) {
				...
				<div class="action-tab warning">
					...
					<p><?php esc_html_e('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'uncode-lite'); ?></p>
